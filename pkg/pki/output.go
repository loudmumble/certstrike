...
func EnumerateAll(cfg *ADCSConfig) (EnumerationResult, error) {
	...
	// Step 2: ESC5 — CA object ACL inspection
	stealthDelay(cfg)
	...
	result.VulnCount += len(result.ESC5Findings) + len(result.ESC8Findings) +
		len(result.ESC9Findings) + len(result.ESC10Findings) + len(result.ESC11Findings) +
